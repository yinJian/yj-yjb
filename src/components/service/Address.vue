<template>
	<div class="address-container" v-if="addressChooseShow">
		...
		<div class="address-area" v-show="showLevel == 3">
			<div class="address-list">
				<div class="address-item" :class="area.selected ? 'address-item-active' : ''" v-for="(area, index) in area" @click="areaChooseHandler(area)">{{ area.codeValue }}</div>
			</div>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			cityChooseHandler (city) {
				...
				getChina(city.codeKey).then(data => {
					data.data.forEach(item => {
						item.selected = false;
					});
					this.area = data.data;
			...
			areaChooseHandler (area) {
				this.areaChoosed = `${area.codeKey},${area.codeValue}`;
				this.$emit('addressConfirm', {
					provinceChoosed: this.provinceChoosed,
					cityChoosed: this.cityChoosed,
					areaChoosed: this.areaChoosed
				});
				this.cancelChoose();
			},
			backChoose () {
